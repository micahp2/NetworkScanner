...
- Persistent device inventory on startup (cached rows load immediately)
- Online/State behavior corrected across rescans (Live/Cached + Yes/No)
- Custom Name / First Seen / Last Seen columns in active workflow
- Column controls: show/hide + order persistence between app restarts
- Corrupt column-layout JSON fallback to safe defaults
...
| | |
|---|---|
| dY"? **Full subnet scan** | Enumerates every host in a CIDR range (e.g. `192.168.1.0/24`) plus additional devices from the OS connection tables |
| dY-� **Hostname resolution** | Reverse DNS for every live host |
| dY"O **MAC address lookup** | Three-layer resolution: modern neighbor table (GetIpNetTable2) �+' legacy ARP cache �+' active SendARP probe |
| dYO? **IPv6 / NDP** | Reads the Windows IPv6 neighbor table to surface MACs for devices that silently drop IPv4 ARP (common on managed Wi-Fi networks) |
| dY?- **Vendor identification** | OUI lookup via macvendors.com with per-prefix caching and rate-limit handling |
| dY"" **Port scanning** | Configurable port list with comma and range syntax (`22,80,443,8080-8090`) |
| dYO` **Dark mode** | Follows the Windows system theme; dark title bar via DWM |
| dY"< **Context menu** | Copy any field A� Browse open ports in the browser A� Open SSH/RDP/terminal from any row |
| dY"Z **Find / search** | Popup search bar (Ctrl+F) with live highlighting and �+`/�+" navigation across all columns |
| �+ **Sortable columns** | All columns; IP Address sorts numerically |
| dY"? **CSV export** | All fields including IPv6 |
| dY"" **Sound events** | Windows notification sound on scan complete; critical stop sound when cancelled |
| dY", **Live theme switching** | Responds instantly to Windows dark/light mode toggle �?" no restart needed |
| dYO? **Multi-range scanning** | Scan multiple non-contiguous subnets in one pass: `192.168.2.0/24, 192.168.4.0/24` |
...
NetworkScanner/
�"o�"?�"? models/
�",   �"o�"?�"? ScanResult.cs          # Result model with INotifyPropertyChanged
�",   �""�"?�"? ScanOptions.cs         # Scan configuration
�"o�"?�"? services/
�",   �"o�"?�"? IPHelperAPI.cs         # P/Invoke: GetIpNetTable2, GetIpNetTable,
�",   �",                          #   SendARP, GetExtendedTcpTable/UdpTable,
�",   �",                          #   DWM, winmm (sounds)
�",   �""�"?�"? NetworkScannerService.cs  # 6-phase scan engine
�"o�"?�"? App.xaml / App.xaml.cs    # Theme (dark/light), DWM title bar, sounds
�"o�"?�"? MainWindow.xaml            # UI layout, column definitions
